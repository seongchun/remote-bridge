...
if [ ${#missing[@]} -gt 0 ]; then
  echo "[mac-mini] MISSING ENV: ${missing[*]} — set these in Claude Code environment config."
  exit 0
fi

STATUS_LINE="$(node scripts/status.mjs --quiet 2>/dev/null || echo 'UNKNOWN')"
echo "[mac-mini] $STATUS_LINE"

# If online, also list sessions
if [[ "$STATUS_LINE" == OK* ]]; then
  echo "[mac-mini] available sessions:"
